...
class GreaseLMModel(GreaseLMPreTrainedModel):
    ...
    def forward(
        ...
        """
        :param input_ids:
              (`torch.LongTensor` of shape `(batch_size, seq_len)`):
        ...
        :param concept_ids:
            (`torch.LongTensor` of shape `(batch_size, number_of_choices, max_node_num)`):
                 Resolved conceptnet ids.
        :param node_type_ids:
              (`torch.LongTensor` of shape `(batch_size, number_of_nodes)`):
                   0 == question entity; 1 == answer choice entity; 2 == other node; 3 == context node
        :param node_scores:
               (`torch.FloatTensor` of shape `(batch_size, number_of_choices, max_node_num, 1)`):
                 LM relevancy scores for each resolved conceptnet id
        :param adj_lengths:
            (`torch.LongTensor` of shape `(batch_size, number_of_choices)`):
                 Adjacency matrix lengths for each batch sample.
        :param special_nodes_mask:
              (`torch.BoolTensor` of shape `(batch_size, number_of_nodes)`):
                 Mask identifying special nodes in the graph (interaction node in the GreaseLM paper).
        :param edge_index:
        ...
        :param position_ids:
             (`torch.LongTensor` of shape `(batch_size, seq_len)`, *optional*, defaults to `None`):
                 Indices of positions of each input sequence tokens in the position embeddings.
        :param head_mask:
        ...
        :param output_attentions: (`bool`) Whether or not to return the attentions tensor.
        :param output_hidden_states: (`bool`, *optional*, defaults to `True`):
...
class GreaseLMForMultipleChoice(GreaseLMPreTrainedModel):
    ...
    def forward(
        ...
        """
        :param input_ids:
              (`torch.LongTensor` of shape `(batch_size, number_of_choices, seq_len)`):
                   Input ids for the language model.
        :param attention_mask:
              (`torch.LongTensor` of shape `(batch_size, number_of_choices, seq_len)`):
                   Attention mask for the language model.
        ...
        :param adj_lengths:
              (`torch.LongTensor` of shape `(batch_size, number_of_choices)`):
                   Adjacency matrix lengths for each batch sample.
        :param special_nodes_mask:
              (`torch.BoolTensor` of shape `(batch_size, number_of_choices, max_node_num)`):
                   Mask identifying special nodes in the graph (interaction node in the GreaseLM paper).
        :param edge_index:
               torch.tensor(2, E)) where E is the total number of edges in the particular graph.
        :param edge_type:
